...
export async function GET({ params, request }) {
  ...
  if (!userToken)
    return json({ error: "X-Spotify-Token manquant" }, { status: 400 });
  ...
  try {
    ...
    if (!plRes.ok) {
      if (plRes.status === 401)
        return json(
          { error: "Token Spotify expir\u00e9 \u2014 reconnecte-toi." },
          { status: 401 },
        );
      if (plRes.status === 404)
        return json({ error: "Playlist introuvable." }, { status: 404 });
    ...
    while (allItems.length < Math.min(loopTotal, 1000)) {
      ...
      if (typeof page.total === "number")
        loopTotal = Math.min(page.total, 1000);
    ...
    if (!tracks.length)
      return json(
        {
          error: `Aucune piste r\u00e9cup\u00e9r\u00e9e (${rawItems.length} items bruts). Playlist vide ou priv\u00e9e ?`,
        },
        { status: 422 },
      );
