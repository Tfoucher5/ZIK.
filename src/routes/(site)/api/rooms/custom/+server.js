...
export async function POST({ request }) {
  ...
  const { name, emoji, tracks, maxRounds = 10, roundDuration = 30, breakDuration = 7 } = await request.json();
  ...
  setTimeout(() => { delete customRooms[code]; delete roomGames[code]; }, 4 * 60 * 60 * 1000);
